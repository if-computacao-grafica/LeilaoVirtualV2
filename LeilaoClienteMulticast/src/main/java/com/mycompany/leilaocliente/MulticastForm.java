...
public class MulticastForm extends javax.swing.JFrame {
    ...
    private void btn_joinMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btn_joinMouseClicked
        ...
        if (bidder.joinAuction()) {
            txt_product_name.setText(bidder.getAuction().getProduto().getNome());
            updateLastBid();
    ...
    private ClienteLicitante createBidder() {
        int index = cmb_auction_list.getSelectedIndex();
        return new ClienteLicitante(txt_chavePublica.getText(), bidderCandidate.getAuctions().get(index));
    }
    ...
    private void setAuctionList() {
        ...
        bidderCandidate.getAuctions().forEach((auction) -> {
            cmb_auction_list.addItem(auction.getProduto().getNome());
        });
    }

    private void updateLastBid() {
        txt_last_bidder.setText(bidder.getAuction().getUltimoLance().getNomeParticipante());
